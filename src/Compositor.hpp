...
class CCompositor {
  public:
    ...
    PHLMONITOR                getMonitorFromVector(const Hyprutils::Math::Vector2D&);
    ...
    PHLWINDOW                 vectorToWindowUnified(const Hyprutils::Math::Vector2D&, uint8_t properties, PHLWINDOW pIgnoreWindow = nullptr);
    SP<CWLSurfaceResource>    vectorToLayerSurface(const Hyprutils::Math::Vector2D&, std::vector<PHLLSREF>*, Hyprutils::Math::Vector2D*, PHLLS*, bool aboveLockscreen = false);
    SP<CWLSurfaceResource>    vectorToLayerPopupSurface(const Hyprutils::Math::Vector2D&, PHLMONITOR monitor, Hyprutils::Math::Vector2D*, PHLLS*);
    SP<CWLSurfaceResource>    vectorWindowToSurface(const Hyprutils::Math::Vector2D&, PHLWINDOW, Hyprutils::Math::Vector2D& sl);
    Hyprutils::Math::Vector2D vectorToSurfaceLocal(const Hyprutils::Math::Vector2D&, PHLWINDOW, SP<CWLSurfaceResource>);
    ...
    PHLWINDOW   getWindowInDirection(const Hyprutils::Math::CBox& box, PHLWORKSPACE pWorkspace, char dir, PHLWINDOW ignoreWindow = nullptr, bool useVectorAngles = false);
    ...
    bool        isPointOnAnyMonitor(const Hyprutils::Math::Vector2D&);
    bool        isPointOnReservedArea(const Hyprutils::Math::Vector2D& point, const PHLMONITOR monitor = nullptr);
    ...
    void        warpCursorTo(const Hyprutils::Math::Vector2D&, bool force = false);
    PHLLS       getLayerSurfaceFromSurface(SP<CWLSurfaceResource>);
    void        closeWindow(PHLWINDOW);
    Hyprutils::Math::Vector2D           parseWindowVectorArgsRelative(const std::string&, const Hyprutils::Math::Vector2D&);
